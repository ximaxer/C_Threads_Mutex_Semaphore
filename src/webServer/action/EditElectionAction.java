...
public class EditElectionAction extends ActionSupport implements SessionAware {
    ...
    private String election, descricao, instituicao, dataI, dataF;
    ...
    public String execute() throws Exception {
        this.election=this.getPrimesBean().getElection();
        System.out.printf("Titulo: %s\nDescricao: %s\nInstituicao: %s\nData Inicial: %s\nData Final: %s\n-----------------------\n",election , descricao, instituicao, dataI, dataF);
        ...
        if (!election.equals("") && !descricao.equals("") && !instituicao.equals("") &&
                ...
                (mesf<=12 && mesf>=1 && diaf<=31 && diaf>=1 && anof<=2100 && anof>=1980 && horaf<=23 && horaf>=0 && minutof<=59 && minutof>=0 ))
        {
            ...
            if (this.getPrimesBean().updateElection(election,descricao,instituicao,dataInicio,dataFim).equals("success")) {
